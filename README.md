...
Below is the conceptual Java implementation highlighting the use of the ANTLR library for normalization and the cache lookup logic.

### 1. Query Normalization & Parameter Binding
```java
...
public class QueryNormalizer extends SQLiteParserBaseListener {
    ...
    public void visitTerminal(TerminalNode node) {
        ...
        // Intercept constants and transform into placeholders
        if (type == SQLiteLexer.NUMERIC_LITERAL || type == SQLiteLexer.STRING_LITERAL) {
            normalizedQuery.append("? "); 
            parameters.add(node.getText()); // Bind the parameter
    ...
    public String getNormalizedSql() {
        // Strip whitespaces and uppercase to guarantee structural matching
        return normalizedQuery.toString().trim().replaceAll("\\s+", " ").toUpperCase();
    }
}
```

### 2. Cache Lookup and Reuse Logic
```java
// QueryPlanCache.java
public CacheResult getPlan(String rawSql) {
    // 1. ANTLR Parsing & Normalization
    SQLiteLexer lexer = new SQLiteLexer(CharStreams.fromString(rawSql));
    ...
    // The Cache Key is the normalized query (e.g. SELECT * FROM USERS WHERE ID = ?)
    String key = normalizer.getNormalizedSql();

    // 2. Cache Lookup
    if (cache.containsKey(key)) {
        // CACHE HIT: Reuse existing plan
        return new CacheResult(cache.get(key), normalizer.getParameters(), true);
    } else {
        // CACHE MISS: Generate new plan via Database Engine
        String jsonPlan = databaseEngine.explainQueryPlan(key);
        cache.put(key, jsonPlan);
        return new CacheResult(jsonPlan, normalizer.getParameters(), false);
    }
}
```
