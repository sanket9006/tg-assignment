...
| :--- | :--- | :--- | :--- |
| **1. Design & Strategy** | Strategy for "Similar" queries | **Completed** | Structural normalization via placeholders, alphabetical column/condition sorting, and 64-bit SHA-256 hashing. |
| | Storing & Looking up plans | **Completed** | Implemented using `ConcurrentHashMap<Long, CachedEntry>` for memory-efficient $O(1)$ lookups. |
...
5. **AST Signatures (Structural Equivalence)**: **Completed** - Implemented a Visitor-based normalization that sorts `SELECT` columns and `WHERE` conditions. `SELECT a, b` and `SELECT b, a` now resolve to the identical cache key.
...
* **The Collections Framework**: Understanding how Java stores data in memory. `ConcurrentHashMap<Long, CachedEntry>` is specifically used because it is thread-safe, meaning if 1,000 users hit your API at once, the cache won't crash. We use `Long` as the key (64-bit hash) to save memory compared to raw strings.
...
4. **The Visitor Pattern**: Your `QueryNormalizer` extends `SQLiteParserBaseVisitor`. Unlike the Listener pattern (which is passive), the Visitor pattern gives us **full control** over how we traverse the tree. We use this to:
   * **Sort SELECT Columns**: Alphabetically reorder projected columns so the order doesn't change the key.
   * **Sort WHERE Conditions**: Reorder `AND`/`OR` clauses so `age > 10 AND name = 'S'` matches `name = 'S' AND age > 10`.
   * **Strip Literals**: Replace numbers and strings with `?` placeholders.
5. **64-bit Hashing**: Instead of using the normalized SQL string itself as a key (which is long and slow to compare), we extract the first 8 bytes of a SHA-256 hash. This gives us a 64-bit `long` which is extremely fast for hashing and memory-efficient.
6. **Metadata Persistence**: We store results in a `CachedEntry` object. This allows us to track "Metadata" like the version of the normalizer used, the extracted arguments, and the exact timestamp of the cache entry.
